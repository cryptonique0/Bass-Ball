/**
 * Replay Engine System - Match replay, clip generation, highlight management
 */
  ...
  recordView(replayId: string, watchTime: number): void {
    ...
    if (replay) {
      ...
      if (analytics) {
        ...
        analytics.completionRate = replay.duration > 0 ? (watchTime / replay.duration) * 100 : 0;
      }
      this.saveToStorage();
    }
  }
  ...
  autoGenerateHighlights(replayId: string, userId: string): Highlight[] {
    ...
    replay.events.forEach(event => {
      if (event.type === 'goal') {
        ...
        highlights.push(this.createHighlight(userId, {
          replayId,
